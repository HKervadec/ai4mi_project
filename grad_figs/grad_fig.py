...
if __name__ == "__main__":
    ...
    dice_grad = -2 * (gt * union - inter) / (union**2) - 2 * ((1 - gt) * n_union - n_inter) / (n_union**2)
    dicedb = np.log10(np.abs(dice_grad).max() / np.abs(dice_grad).min())
    print(f"{dicedb=}")

    l2_grad = 2 * (noisy_softmax - gt)

    ce_grad = - (1 - gt) / ((1 - noisy_softmax) + 1e-10) - (gt) / ((noisy_softmax) + 1e-10)
    ...
    # DSC grad
    fig = plt.figure()
    im = plt.imshow(dice_grad, cmap='jet', interpolation='nearest', vmin=dice_grad.min(), vmax=dice_grad.max())
    plt.axis('off')
    plt.colorbar(im)
    ...
    im = plt.imshow(ce_grad, cmap='jet', interpolation='nearest', norm=SymLogNorm(linthresh=0.03,
                                                                                  vmin=ce_grad.min(),
                                                                                  # vmax=ce_grad.max(
                                                                                  vmax=0))
    plt.axis('off')
