...
data/TOY2:
	rm -rf $@_tmp $@
	python src/scripts/gen_two_circles.py --dest $@_tmp -n 1000 100 -r 25 -wh 256 256
	mv $@_tmp $@
...
data/SEGTHOR:
	...
	rm -rf $@_tmp $@
	python $(CFLAGS) src/scripts/slice_segthor.py --source_dir data/segthor_part1 --dest_dir $@_tmp \
